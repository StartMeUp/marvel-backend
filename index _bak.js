...
app.get("/characters/:id", async (req, res) => {
  try {
    //const response = await axios(apiUrl("characters", req.params.id));
    const response = await axios(
      apiUrl({ endpoint: "characters", id: req.params.id })
    );
...
app.get("/comics/:id", async (req, res) => {
  try {
    const response = await axios(apiUrl("comics", req.params.id));
    res.status(200).json(response.data);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
app.get("/comics", async (req, res) => {
  try {
    const response = await axios(apiUrl("comics"));
    res.status(200).json(response.data);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
